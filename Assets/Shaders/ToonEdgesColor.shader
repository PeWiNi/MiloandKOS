Shader "Hard-edged toon - fixed colour" {
	Properties {
		_DiffBaseColor ("Diffuse base colour", COLOR) = (1,1,1,1)
		//_DiffMap1 ("Diffuse map 1", 2D) = "white" {}
		_DiffMap2 ("Diffuse map 2", 2D) = "white" {}
		_DiffMap3 ("Diffuse map 3", 2D) = "white" {}
		_DiffMap4 ("Diffuse map 4", 2D) = "white" {}

		_Threshold1 ("Threshold 1", Range(-1,1)) = 0.6
		_Threshold2 ("Threshold 2", Range(-1,1)) = 0.2
		_Threshold3 ("Threshold 3", Range(-1,1)) = -0.3

		_Overlap1 ("Overlap length 1", Range(0, 0.5)) = 0.05
		_Overlap2 ("Overlap length 2", Range(0, 0.5)) = 0.05
		_Overlap3 ("Overlap length 3", Range(0, 0.5)) = 0.05

		_ScaleOffset ("Edge thickness", VECTOR) = (0.03, 0.05, 0.15, 0)
		_PivotOffset ("Pivot point offset", VECTOR) = (0, 0.5, 0.05, 0)
	}
	SubShader {
		Pass{
			Cull Back
			tags{"LightMode" = "ForwardBase"}
			Fog {Mode OFF}
			
			CGPROGRAM
			#pragma vertex vert
			#pragma fragment frag

			uniform float4 _OutlineColour;
			uniform float4 _ScaleOffset;
			uniform float4 _PivotOffset;

			struct VertexIn
			{
				float4 pos : POSITION;
			};

			struct VertexOut
			{
				float4 pos : SV_POSITION;
			};

			VertexOut vert(VertexIn input)
			{
				VertexOut output;
				
				float tempX = (input.pos.x - _PivotOffset.x) * (1 + _ScaleOffset.x) + _PivotOffset.x;
				float tempY = (input.pos.y - _PivotOffset.y) * (1 + _ScaleOffset.y) + _PivotOffset.y;
				float tempZ = (input.pos.z - _PivotOffset.z) * (1 + _ScaleOffset.z) + _PivotOffset.z;
				output.pos = mul(UNITY_MATRIX_MVP, float4(tempX, tempY, tempZ, 1));

				return output;
			}

			float4 frag(VertexOut input) : COLOR
			{
				return _OutlineColour;
			}

			ENDCG
		}

		Pass {
			Cull Back
			Blend One Zero

			CGPROGRAM

			#pragma vertex vert
			#pragma fragment frag

			#pragma target 3.0

			#include "UnityCG.cginc"

			uniform float4 _DiffBaseColor;
			uniform sampler2D _DiffMap1;
			uniform sampler2D _DiffMap2;
			uniform sampler2D _DiffMap3;
			uniform sampler2D _DiffMap4;
			uniform float _Threshold1;
			uniform float _Threshold2;
			uniform float _Threshold3;
			uniform float _Overlap1;
			uniform float _Overlap2;
			uniform float _Overlap3;

			struct vIn 
			{
				float4 pos : POSITION;
				float4 normal : NORMAL;
				float4 tex : TEXCOORD0;
			};

			struct v2f 
			{
				float4 pos : SV_POSITION;
				//float2 pos2 : TEXCOORD3; // Used for drawing textures relative to screen rather than world space.
				float4 normal : TEXCOORD0;
				float4 lightDirection : TEXCOORD1;
				float4 tex : TEXCOORD2;
			};

			v2f vert(vIn input)
			{
				v2f output;

				output.pos = mul(UNITY_MATRIX_MVP, input.pos);
				output.normal = mul(input.normal, _World2Object);
				output.lightDirection = normalize(_WorldSpaceLightPos0);
				output.tex = input.tex;
				//output.pos2 = float2(output.pos.x / output.pos.w, output.pos.y / output.pos.w);

				return output;
			}

			float4 frag(v2f input) : COLOR
			{
				float dotProduct = dot(input.normal, input.lightDirection);

				if(dotProduct > _Threshold1)
				{
					return _DiffBaseColor;
				}
				else if(dotProduct > (_Threshold1 - _Overlap1))
				{
					float mix = (_Threshold1 - dotProduct) / _Overlap1; // Goes from 0 to 1
					return 						 _DiffBaseColor 
						* lerp( float4(1,1,1,1), tex2D(_DiffMap2, float2(input.tex.xy)), mix);
				}
				else if(dotProduct > _Threshold2)
				{
					return _DiffBaseColor 
						 * tex2D(_DiffMap2, float2(input.tex.xy));
				}
				else if(dotProduct > (_Threshold2 - _Overlap2))
				{
					float mix = (_Threshold2 - dotProduct) / _Overlap2; 
					return 						 _DiffBaseColor
						 					   * tex2D(_DiffMap2, float2(input.tex.xy)) 
						* lerp( float4(1,1,1,1), tex2D(_DiffMap3, float2(input.tex.xy)), mix);
				}
				else if(dotProduct > _Threshold3)
				{
					return _DiffBaseColor
						 * tex2D(_DiffMap2, float2(input.tex.xy)) 
						 * tex2D(_DiffMap3, float2(input.tex.xy));
				}
				else if(dotProduct > (_Threshold3 - _Overlap3))
				{
					float mix = (_Threshold3 - dotProduct) / _Overlap3; 
					return 						 _DiffBaseColor 
						 					   * tex2D(_DiffMap2, float2(input.tex.xy))
						 					   * tex2D(_DiffMap3, float2(input.tex.xy)) 
						* lerp( float4(1,1,1,1), tex2D(_DiffMap4, float2(input.tex.xy)), mix);
				}
				else
				{
					return _DiffBaseColor
						 * tex2D(_DiffMap2, float2(input.tex.xy)) 
						 * tex2D(_DiffMap3, float2(input.tex.xy)) 
						 * tex2D(_DiffMap4, float2(input.tex.xy));
				}
			}
		
			ENDCG
		}
	} 
	FallBack "Diffuse"
}
